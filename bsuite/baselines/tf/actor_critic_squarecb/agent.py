...
class ActorCriticSquareCB(base.Agent):
  ...
  def __init__(
    ...
    self.values = [0]*obs_spec.shape[0]

  @tf.function
  def _sample_policy(self, inputs: tf.Tensor) -> tf.Tensor:
    policy, value = self._network(inputs)
    num_actions = policy.logits.shape[-1]
    probabilities = [policy.prob(tf.constant(i, dtype=tf.int32)) for i in range(num_actions)]
    
    action = policy.sample()
    return tf.squeeze(action), probabilities, value
  ...
  def select_action(self, timestep: dm_env.TimeStep) -> base.Action:
    ...
    print(observation)
    action, probabilities, value = self._sample_policy(observation)
    # final_probs = []
    # for prob in probabilities:
    #   final_probs.append(prob.numpy()[0])
    # print(final_probs)

    # observation = tf.expand_dims(timestep.observation, axis=0)
    # policy, value = self._network(observation)
    # print(value)


    #insert squarecb algo
    return action.numpy()
...
class PolicyValueNet(snt.Module):
  ...
  def __call__(self, inputs: tf.Tensor) -> Tuple[tfp.distributions.Distribution, tf.Tensor]:
